...
如果要把 SDD 内容通过 Git 同步到外部系统展示，建议优先展示这个文件，然后从这里跳转到每个 feature 的 `spec.md`、`plan.md`、`tasks.md` 和 `quickstart.md`。
...
| `quickstart.md` | 验证说明 | 记录如何启动、如何验证、预期结果是什么 |
...
本项目当前已经通过 SDD 落地以下后台管理基础能力：

- Ant Design Pro 登录
- Kratos 当前用户接口
- Higress `/api/*` 路由转发
- 后台品牌名 `go-ant-design-pro-admin`
...
- 角色绑定菜单权限和按钮权限
- 用户绑定角色
- `currentUser` 返回角色编码、菜单权限、按钮权限和授权菜单树
- 数据库菜单名称、排序、状态和授权显隐联动左侧菜单

## 推荐展示方式

如果后续做 Git 同步和任务可视化，可以按这个优先级读取：

1. 展示 [本文件](README.zh-CN.md) 作为 SDD 首页。
...
| --- | --- |
| 功能编号 | 目录名前缀，例如 `006` |
| 功能名称 | 目录名或 `spec.md` 标题 |
| 需求状态 | `spec.md` 的 Status 字段 |
| 任务总数 | `tasks.md` 中 `- [ ]` 和 `- [x]` 数量 |
| 已完成数 | `tasks.md` 中 `- [x]` 数量 |
| 未完成数 | `tasks.md` 中 `- [ ]` 数量 |
| 验证入口 | `quickstart.md` |
| 技术决策 | `research.md` |
...
| 大改动，例如权限模型重构、JWT 网关鉴权 | 新建新的 feature 目录，例如 `008-gateway-jwt-auth` |

变更流程：

```text
更新 spec.md
必要时更新 contracts/ 和 data-model.md
更新 plan.md
更新或追加 tasks.md
实现任务
更新 quickstart.md / research.md
完成验证后勾选 tasks.md
```
