...
5. `docs/frontend/ant-design-pro-conventions.md`
   - 前端页面、组件、权限、请求和 UI 规范。

6. `docs/frontend/design.md`
   - Ant Design 官方 `design.md` 本地副本；涉及 UI、页面、组件、主题 Token 和视觉语言时先读。

7. `docs/backend/kratos-conventions.md`
   - Kratos API、分层、生成、错误、配置和验证规范。
...
- 先定义契约，再写整合代码。
- 保持模块可单独启动和验证。
- 前端变更优先遵循 Ant Design Pro、Ant Design、ProComponents 和 Umi Max 约定。
- 涉及 UI 设计、页面布局、组件视觉和主题 Token 时，优先读取 `docs/frontend/design.md`，不依赖联网读取官网。
- 后端变更优先遵循 Kratos protobuf-first、HTTP/gRPC、Wire、service/biz/data 分层约定。
